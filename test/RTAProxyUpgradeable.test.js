...
describe("RTAProxyUpgradeable Multi-Sig", function () {
    ...
    describe("Internal Wallet Time-Lock Bypass", function () {
        let treasury, externalWallet;

        beforeEach(async function () {
            [, , , , , , treasury, externalWallet] = await ethers.getSigners();

            // Add treasury as internal wallet through multi-sig
            const addWalletData = rtaProxy.interface.encodeFunctionData("addInternalWallet", [treasury.address]);
            await rtaProxy.connect(signer1).submitOperation(rtaProxyAddress, addWalletData, 0);
            await rtaProxy.connect(signer2).confirmOperation(0);

            // Mint tokens to holder1 for testing transfers
            ...
            await rtaProxy.connect(signer1).submitOperation(tokenAddress, mintData, 0);
        ...
        it("Should NOT require time-lock for high-value transfers to internal wallets", async function () {
            const highValue = ethers.parseUnits("1500000", 10); // 1.5M tokens (above threshold)
            ...
            // Time-lock should NOT be required for internal wallet
        ...
        it("Should REQUIRE time-lock for high-value transfers to external wallets", async function () {
            ...
            // Encode transferFromRegulated to external wallet
            const transferData = token.interface.encodeFunctionData("transferFromRegulated", [
                holder1.address,
            ...
            // Time-lock SHOULD be required for external wallet
            expect(await rtaProxy.requiresTimeLock(transferData)).to.be.true;
        });

        it("Should execute high-value transfer to internal wallet immediately", async function () {
            const highValue = ethers.parseUnits("1500000", 10); // 1.5M tokens

            // Submit transfer to internal wallet (treasury)
            const transferData = token.interface.encodeFunctionData("transferFromRegulated", [
                holder1.address,
                treasury.address,
                highValue,
            ...
            await rtaProxy.connect(signer1).submitOperation(tokenAddress, transferData, 0);

            // Should execute immediately with second confirmation (no time-lock)
            await expect(rtaProxy.connect(signer2).confirmOperation(2))
                .to.emit(rtaProxy, "OperationExecuted")
                .withArgs(2);

            // Verify transfer completed
            expect(await token.balanceOf(treasury.address)).to.equal(highValue);
        });

        it("Should NOT require time-lock for small transfers regardless of destination", async function () {
            const lowValue = ethers.parseUnits("500000", 10); // 500K tokens (below threshold)

            // External wallet, but small amount
            const transferData = token.interface.encodeFunctionData("transferFromRegulated", [
                holder1.address,
                externalWallet.address,
                lowValue,
            ...
            // Time-lock should NOT be required (amount below threshold)
            expect(await rtaProxy.requiresTimeLock(transferData)).to.be.false;
        });

        it("Should manage internal wallet registry correctly", async function () {
            // Treasury was added in beforeEach
            ...
            // Remove treasury from internal wallet registry
            // beforeEach created operations 0 and 1, so this will be operation 2
            const removeData = rtaProxy.interface.encodeFunctionData("removeInternalWallet", [treasury.address]);
            await rtaProxy.connect(signer1).submitOperation(rtaProxyAddress, removeData, 0);
        ...
        it("Should require time-lock after wallet is removed from internal registry", async function () {
            const highValue = ethers.parseUnits("1500000", 10);

            // Initially, transfers to treasury don't need time-lock
            const transferData = token.interface.encodeFunctionData("transferFromRegulated", [
                holder1.address,
                treasury.address,
                highValue,
            ...
            // Now transfers to treasury SHOULD require time-lock
            expect(await rtaProxy.requiresTimeLock(transferData)).to.be.true;
        });
    });
